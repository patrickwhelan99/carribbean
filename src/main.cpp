...
int gameMain(sf::RenderWindow &app, int &gridSize, uint32_t &seed, std::string &playerName, unsigned &threads)
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        alphaTexture.loadFromFile(std::string("textures/alpha_" + std::to_string(i) + ".png"));
...
/// Perform initial Pathfinidng on generated grid
    ...
    if(!landBlendShader.loadFromFile("shaders/landBlendShader.frag", sf::Shader::Fragment))
